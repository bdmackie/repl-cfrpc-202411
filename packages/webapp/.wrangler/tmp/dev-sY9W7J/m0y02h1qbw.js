...
// .wrangler/tmp/pages-d0qgfW/bundledWorker-0.9366450401221305.mjs
import { Writable as Writable2 } from "node:stream";
var __defProp2 = Object.defineProperty;
var __defNormalProp2 = /* @__PURE__ */ __name((obj, key2, value) => key2 in obj ? __defProp2(obj, key2, { enumerable: true, configurable: true, writable: true, value }) : obj[key2] = value, "__defNormalProp");
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var __publicField2 = /* @__PURE__ */ __name((obj, key2, value) => {
...
var _Performance2 = /* @__PURE__ */ __name(class {
  ...
  mark(name, options2) {
    const entry = new _PerformanceMark2(name, options2);
    this._entries.push(entry);
    return entry;
  }
  measure(measureName, startOrMeasureOptions, endMark) {
...
var __esm = /* @__PURE__ */ __name2((fn22, res) => /* @__PURE__ */ __name2(/* @__PURE__ */ __name(function __init() {
...
function create_effect(type, fn22, sync, push2 = true) {
...
function effect_root(fn22) {
  const effect2 = create_effect(ROOT_EFFECT, fn22, true);
  return () => {
    destroy_effect(effect2);
  };
}
...
function effect(fn22) {
  return create_effect(EFFECT, fn22, false);
}
__name(effect, "effect");
__name2(effect, "effect");
function branch(fn22, push2 = true) {
  return create_effect(RENDER_EFFECT | BRANCH_EFFECT, fn22, true, push2);
}
...
function handle_error(error32, effect2, component_context2) {
  {
    throw error32;
  }
}
...
function push$1(props, runes = false, fn22) {
  component_context = {
    p: component_context,
    c: null,
    e: null,
...
__name2(escape_html2, "escape_html2");
var ATTR_REGEX;
var CONTENT_REGEX;
...
__name2(getPlatform, "getPlatform");
var load;
var init_page_server_ts = __esm({
  ".svelte-kit/output/server/entries/pages/sum/_page.server.ts.js"() {
    ...
    load = /* @__PURE__ */ __name2(async (event) => {
      const result1 = 1 + 2;
      const platform32 = await getPlatform(event);
      const result2 = await platform32.env.SUM_WORKER.add(1, 2);
...
function handle_event_propagation(event) {
  ...
  try {
      ...
      try {
        ...
        current_target.disabled) {
          if (is_array(delegated)) {
            var [fn22, ...data] = delegated;
            fn22.apply(current_target, [event, ...data]);
          } else {
    ...
    if (throw_error) {
      for (let error32 of other_errors) {
        queueMicrotask(() => {
          throw error32;
        });
      }
...
function unmount(component4) {
  const fn22 = mounted_components.get(component4);
  if (fn22) {
    fn22();
  }
}
...
function set_read_implementation(fn22) {
  read_implementation = fn22;
}
...
function asClassComponent(component4) {
  ...
  const _render = /* @__PURE__ */ __name2((props, { context: context22 } = {}) => {
...
var options = {
  ...
  templates: {
    app: ({ head: head2, body: body2, assets: assets2, nonce, env: env32 }) => '<!doctype html>\n<html lang="en">\n	<head>\n		<meta charset="utf-8" />\n		<link rel="icon" href="' + assets2 + '/favicon.png" />\n		<meta name="viewport" content="width=device-width, initial-scale=1" />\n		' + head2 + '\n	</head>\n	<body data-sveltekit-preload-data="hover">\n		<div style="display: contents">' + body2 + "</div>\n	</body>\n</html>\n",
...
function writable(value, start = noop22) {
  let stop = null;
  ...
  function subscribe(run, invalidate = noop22) {
    const subscriber = [run, invalidate];
    subscribers.add(subscriber);
    if (subscribers.size === 1) {
      stop = start(set2, update) || noop22;
    }
    run(
      /** @type {T} */
      value
...
var HttpError = /* @__PURE__ */ __name2(class {
  /**
   * @param {number} status
...
var Redirect = /* @__PURE__ */ __name2(class {
  /**
...
var SvelteKitError = /* @__PURE__ */ __name2(class extends Error {
  /**
   * @param {number} status
   * @param {string} text
...
var ActionFailure = /* @__PURE__ */ __name2(class {
  /**
   * @param {number} status
   * @param {T} data
   */
...
function normalize_error(error32) {
  return (
...
function get_status(error32) {
  return error32 instanceof HttpError || error32 instanceof SvelteKitError ? error32.status : 500;
}
__name(get_status, "get_status");
__name2(get_status, "get_status");
function get_message(error32) {
  return error32 instanceof SvelteKitError ? error32.text : "Internal Error";
}
...
async function handle_fatal_error(event, options2, error32) {
  error32 = error32 instanceof HttpError ? error32 : coalesce_to_error(error32);
  const status = get_status(error32);
  const body2 = await handle_error_and_jsonify(event, options2, error32);
...
async function handle_error_and_jsonify(event, options2, error32) {
  if (error32 instanceof HttpError) {
    return error32.body;
  }
  const status = get_status(error32);
  const message = get_message(error32);
  return await options2.hooks.handleError({ error: error32, event, status, message }) ?? { message };
}
...
function clarify_devalue_error(event, error32) {
  if (error32.path) {
    return `Data returned from \`load\` while rendering ${event.route.id} is not serializable: ${error32.message} (data${error32.path})`;
  }
  if (error32.path === "") {
...
function stringify(value, reducers) {
  ...
  function flatten(thing) {
    ...
    for (const { key: key2, fn: fn22 } of custom) {
      const value2 = fn22(thing);
      if (value2) {
...
function check_incorrect_fail_use(error32) {
  return error32 instanceof ActionFailure ? new Error('Cannot "throw fail()". Use "return fail()"') : error32;
}
...
function try_deserialize(data, fn22, route_id) {
  try {
    return fn22(data);
  } catch (e3) {
    const error32 = (
    ...
    if ("path" in error32) {
      let message = `Data returned from action inside ${route_id} is not serializable: ${error32.message}`;
      if (error32.path !== "")
        message += ` (data.${error32.path})`;
...
async function render_response({
  ...
  if (page_config.csr) {
    ...
    if (page_config.ssr) {
      ...
      if (error32) {
        serialized.error = uneval(error32);
      }
      const hydrate2 = [
  ...
  function replacer(thing) {
    if (typeof thing?.then === "function") {
      ...
      thing.then(
        ...
        /** @param {any} error */
        async (error32) => ({
          error: await handle_error_and_jsonify(event, options2, error32)
        })
      ).then(
        /**
         * @param {{data: any; error: any}} result
         */
        async ({ data, error: error32 }) => {
          count32 -= 1;
          let str;
          try {
            str = uneval({ id, data, error: error32 }, replacer);
          } catch {
            error32 = await handle_error_and_jsonify(
              event,
              options2,
...
`);
  ...
  try {
    ...
    return {
      data: `[${strings.join(",")}]`,
      chunks: count32 > 0 ? iterator : null
    };
  } catch (e3) {
...
async function render_data(event, route, options2, manifest2, state, invalidated_data_nodes, trailing_slash) {
  ...
  try {
    ...
    const promises = functions.map(async (fn22, i) => {
      if (!invalidated[i]) {
        return (
          /** @type {import('types').ServerDataSkippedNode} */
          {
    ...
    const nodes = await Promise.all(
      promises.map(
        (p, i) => p.catch(async (error32) => {
          if (error32 instanceof Redirect) {
            throw error32;
          }
          ...
          return (
            /** @type {import('types').ServerErrorNode} */
            {
              type: "error",
              error: await handle_error_and_jsonify(event, options2, error32),
              status: error32 instanceof HttpError || error32 instanceof SvelteKitError ? error32.status : void 0
            }
          );
        })
      )
    ...
    const error32 = normalize_error(e3);
    if (error32 instanceof Redirect) {
      return redirect_json_response(error32);
    } else {
      return json_response(await handle_error_and_jsonify(event, options2, error32), 500);
    }
  }
}
        ...
        thing.catch(
          ...
          async (value) => {
            ...
            try {
              ...
              const error32 = await handle_error_and_jsonify(
                event,
                options2,
                new Error(`Failed to serialize promise while rendering ${event.route.id}`)
              );
              key2 = "error";
              str = stringify(error32, reducers);
            }
            count32 -= 1;
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      ...
      if (node) {
        try {
          ...
          const error32 = await handle_error_and_jsonify(event, options2, err);
          while (i--) {
            if (page2.errors[i]) {
              const index4 = (
                /** @type {number} */
          ...
          return static_error_page(options2, status2, error32.message);
        }
      } else {
        branch2.push(null);
      }
    ...
    if (route) {
      ...
      if (state.before_handle || state.emulator?.platform) {
        ...
        if (route.endpoint) {
          ...
          const nodes = await load_page_nodes(route.page, manifest2);
          config22 = get_page_config(nodes) ?? config22;
          prerender = get_option(nodes, "prerender") ?? false;
        }
        if (state.before_handle) {
          state.before_handle(event, config22, prerender);
        }
        if (state.emulator?.platform) {
          event.platform = await state.emulator.platform({ config: config22, prerender });
        }
      }
...
function filter_private_env(env32, { public_prefix, private_prefix }) {
...
function filter_public_env(env32, { public_prefix, private_prefix }) {
  return Object.fromEntries(
    Object.entries(env32).filter(
...
var Server = /* @__PURE__ */ __name2(class {
  ...
  async init({ env: env32, read }) {
    ...
    const private_env = filter_private_env(env32, prefixes);
    const public_env2 = filter_public_env(env32, prefixes);
    set_private_env(
    ...
    if (!this.#options.hooks) {
      try {
        ...
        this.#options.hooks = {
          ...
          handleError: module.handleError || (({ error: error32 }) => console.error(error32)),
...
var manifest = (() => {
  function __memo(fn22) {
    let value;
    return () => value ??= value = fn22();
  }
...
var worker = {
  async fetch(req, env32, context22) {
    await server.init({ env: env32 });
    ...
    if (is_static_asset || prerendered.has(pathname) || pathname === version_file || pathname.startsWith(immutable)) {
      res = await env32.ASSETS.fetch(req);
      ...
      res = await server.respond(req, {
        // @ts-ignore
        platform: { env: env32, context: context22, caches, cf: req.cf },
        getClientAddress() {
...
function __facade_invokeChain__(request, env32, ctx, dispatch, middlewareChain) {
  ...
  return head2(request, env32, ctx, middlewareCtx);
}
...
function wrapExportedHandler(worker2) {
  ...
  const fetchDispatcher = /* @__PURE__ */ __name2(function(request, env32, ctx) {
    ...
    return worker2.fetch(request, env32, ctx);
  }, "fetchDispatcher");
  return {
    ...worker2,
    fetch(request, env32, ctx) {
      const dispatcher = /* @__PURE__ */ __name2(function(type, init2) {
        if (type === "scheduled" && worker2.scheduled !== void 0) {
          ...
          return worker2.scheduled(controller, env32, ctx);
        }
      }, "dispatcher");
...
// ../../node_modules/.pnpm/wrangler@3.90.0_@cloudflare+workers-types@4.20241112.0/node_modules/wrangler/templates/pages-dev-util.ts
...
// .wrangler/tmp/pages-d0qgfW/m0y02h1qbw.js
  ...
  fetch(request, env4, context3) {
    ...
    for (const include of routes.include) {
      if (isRoutingRuleMatch(pathname, include)) {
        ...
        return middleware_loader_entry_default.fetch(request, env4, context3);
      }
    }
    return env4.ASSETS.fetch(request);
  }
...
// .wrangler/tmp/bundle-yWMDR4/middleware-insertion-facade.js
...
function __facade_invokeChain__2(request, env4, ctx, dispatch, middlewareChain) {
  ...
  return head2(request, env4, ctx, middlewareCtx);
}
...
// .wrangler/tmp/bundle-yWMDR4/middleware-loader.entry.ts
...
function wrapExportedHandler2(worker2) {
  ...
  const fetchDispatcher = /* @__PURE__ */ __name(function(request, env4, ctx) {
    ...
    return worker2.fetch(request, env4, ctx);
  }, "fetchDispatcher");
  return {
    ...worker2,
    fetch(request, env4, ctx) {
      const dispatcher = /* @__PURE__ */ __name(function(type, init2) {
        if (type === "scheduled" && worker2.scheduled !== void 0) {
          ...
          return worker2.scheduled(controller, env4, ctx);
        }
      }, "dispatcher");
